...
function showModal(encodedPath, pages, percentage, encodedeTitle, encodedEntryTitle, titleID, entryID) {
	...
	if (percentage === 0) {
		...
		$('#continue-btn').text('继续阅读 ' + percentage + '%');
	}
...
const renameSubmit = (name, eid) => {
	...
	$.ajax({
		...
		.done(data => {
			if (data.error) {
				alert('danger', `无法更新显示名称. Error: ${data.error}`);
				return;
			}
			location.reload();
		})
		.fail((jqXHR, status) => {
			alert('danger', `无法更新显示名称. Error: [${jqXHR.status}] ${jqXHR.statusText}`);
		});
};
...
const renameSortNameSubmit = (name, eid) => {
	...
	$.ajax({
		...
		.done(data => {
			if (data.error) {
				alert('danger', `未能更新排序标题. Error: ${data.error}`);
				return;
			}
			location.reload();
		})
		.fail((jqXHR, status) => {
			alert('danger', `未能更新排序标题. Error: [${jqXHR.status}] ${jqXHR.statusText}`);
		});
};
...
const setupUpload = (eid) => {
	...
	UIkit.upload('.upload-field', {
		...
		name: 'file',
		error: (e) => {
			alert('danger', `上传封面图片失败: ${e.toString()}`);
		},
		loadStart: (e) => {
...
const bulkProgress = (action, el) => {
	...
	$.ajax({
		...
		.done(data => {
			if (data.error) {
				alert('danger', `未能将条目标记为 ${action}. Error: ${data.error}`);
				return;
			}
			location.reload();
		})
		.fail((jqXHR, status) => {
			alert('danger', `未能将条目标记为 ${action}. Error: [${jqXHR.status}] ${jqXHR.statusText}`);
		})
		.always(() => {
			deselectAll();
		});
...
const tagsComponent = () => {
	return {
		...
		load(admin) {
			...
			$('.tag-select').select2({
				tags: true,
				placeholder: this.isAdmin ? '标记标题' : '未找到标签',
